...
class _MyAppState2 extends State<MyApp> {
  ...
  late Widget page = versions1[1].version=="1.0.8+11"?StartWorkMainScreen():VersionWarningScreen();
  late Widget page2 = versions1[1].version=="1.0.8+11"?NavigationMainScreen():VersionWarningScreen();
  late Widget page3 = versions1[1].version=="1.0.8+11"?LoginMainScreen():VersionWarningScreen();
  ...
  Widget build(BuildContext context) {
    ...
    void pageCondition(){
      ...
      if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isStoreVisit')==true&& isWithinTimeRange && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"==now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = ShopVisitingProcessesScreen(shop_code: box.get('currentShopID'), shopName: box.get('currentShopName'));
      }

      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isRegionCenterVisit')==true&& isWithinTimeRange && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"==now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = RegionCenterVisitingMainScreen(region_code: box.get('currentCenterID'), regionName: box.get('currentCenterName'));
      }

      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isStartShift')==true && isWithinTimeRange && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"==now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = ShiftTypeScreen();
      }

      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isStartShift')==true && isWithinTimeRange2 && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"==now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = StartWorkMainScreen();
        page2 = NavigationMainScreen();
        shiftManager.endShift();
      }
      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isStartShift')==true && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"!=now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = StartWorkMainScreen();
        page2 = NavigationMainScreen();
        shiftManager.endShift();
      }

      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isStoreVisit')==true && isWithinTimeRange2 && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"==now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = StartWorkMainScreen();
        page2 = NavigationMainScreen();
        storeVisitManager.endStoreVisit();
      }
      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isStoreVisit')==true && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"!=now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = StartWorkMainScreen();
        page2 = NavigationMainScreen();
        storeVisitManager.endStoreVisit();
      }

      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isRegionCenterVisit')==true && isWithinTimeRange2 && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"==now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = StartWorkMainScreen();
        page2 = NavigationMainScreen();
        regionCenterVisitManager.endRegionCenterVisit();
      }
      else if(versions1[1].version=="1.0.8+11" && boxStateManagement.get('isRegionCenterVisit')==true && "${int.parse(box.get("shiftDate").split("T")[0].split("-")[2])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[1])}-${int.parse(box.get("shiftDate").split("T")[0].split("-")[0])}"!=now.day.toString()+"-"+now.month.toString()+"-"+now.year.toString()){
        page = StartWorkMainScreen();
        page2 = NavigationMainScreen();
        regionCenterVisitManager.endRegionCenterVisit();
      }

      else if(versions1[1].version!="1.0.8+11"){
        page = VersionWarningScreen();
        page2 = VersionWarningScreen();
      }
    }
